...
fn watch_debounced_line<R: Runtime>(window: Window<R>, rx: Receiver<DebounceEventResult>, id: Id) {
    spawn(move || {
        let event_name = format!("watcher://debounced-event/{id}");

        let path = "/home/pacto/Downloads/test.txt";
...
#[command]
async fn watch_appended<R: Runtime>(
    window: Window<R>,
    watchers: State<'_, WatcherCollection>,
    id: Id,
    path: PathBuf,
